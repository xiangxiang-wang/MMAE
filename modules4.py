...
from utils import patchify, unpatchify, forward_loss, compute_loss, image_padding, image_unpadding,create_mask
...
class MMAE(nn.Module):
    ...
    def forward(self, o1, o2):
        ...
        pre_mask1,pre_mask2,pre_mask3 = create_mask(o1,o2)
        # pre_mask = (pre_mask1+pre_mask2+pre_mask3)/3  #b,1,h,w
        pre_mask = torch.min(pre_mask1,pre_mask2)
        pre_mask =torch.min(pre_mask,pre_mask3)
        pre_mask = self.pool(pre_mask)
        s1 = torch.ones_like(pre_mask)
        s2 = torch.zeros_like(pre_mask)
        pre_mask = torch.where(pre_mask > 0.5, s1, s2)
